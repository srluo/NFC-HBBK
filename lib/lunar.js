...
var jieAlert;
/*----------------------------------------------------------*/
function make_array(...tmparg) {
  for (let i = 0; i < tmparg.length; i++) {
    this[i] = tmparg[i];
  }
}
...
// 讓外部可取得計算後的 gan/zhi/lunar，也可先把天干地支表丟到全域
if (typeof globalThis !== "undefined") {
  globalThis.GanGB = GanGB;
  globalThis.ZhiGB = ZhiGB;
  globalThis.ShengXiaoGB = ShengXiaoGB;

  // 將內部結果指向全域（每次 solar2Lunar() 跑完後都會更新）
  globalThis._ein_gan   = gan;
  globalThis._ein_zhi   = zhi;
  globalThis._ein_lunar = lunar;
  globalThis._ein_solar = solar;
}
